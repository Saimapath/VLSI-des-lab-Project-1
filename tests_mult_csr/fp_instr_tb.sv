...
module tb_fp_soc_step();
    ...
    initial begin
        ...
        // =========================================================================
        // ---> CHANGED: PRE-LOAD DATA MEMORY (Using the 8 Banks)
        // =========================================================================
        // Byte Addr 0  -> Word 0 -> bank0[0]
        // Byte Addr 4  -> Word 1 -> bank1[0]
        // Byte Addr 8  -> Word 2 -> bank2[0]
        // Byte Addr 12 -> Word 3 -> bank3[0]
        dut.dmem.bank0[0] = 32'h00003C00; // addr 0: 1.0
        dut.dmem.bank1[0] = 32'h00004000; // addr 4: 2.0
        dut.dmem.bank2[0] = 32'h0000C000; // addr 8: -2.0
        dut.dmem.bank3[0] = 32'h00004400; // addr 12: 4.0
    ...
    always @(negedge clk) begin
        ...
        if (int_write_retiring) begin
            ...
            if (int_write_rd == 30 && int_write_wd == 1) begin
                $display("----------------------------------------------------------------------------------");
                // =========================================================================
                // ---> CHANGED: CHECK FSW OUTPUT IN BANK 5
                // Address 20 -> Word 5 -> bank5[0]
                // =========================================================================
                if (dut.dmem.bank5[0] === 32'h00004800) 
                    $display("[PASS] Memory FSW Check: 8.0 successfully stored in Data RAM.");
                else 
                    $display("[FAIL] Memory FSW Check: Expected 00004800, Got %h", dut.dmem.bank5[0]);
